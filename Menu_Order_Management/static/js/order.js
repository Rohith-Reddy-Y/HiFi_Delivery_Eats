document.addEventListener("DOMContentLoaded", function () {
    ...
    const currentOrderDetailsContainer = document.getElementById(
    ...
    function updateCartCount() {
      ...
      if (cartLink) {
        const span =
          cartLink.querySelector(".nav__cart-count") ||
        ...
        console.log(
          "localStorage cart after update:",
          JSON.parse(localStorage.getItem("cart"))
        );
      } else {
        console.error(
          "Cart link element (.nav__link[href='order.html']) not found in the DOM."
        );
      }
    }
      ...
      if (cart.length > 0) {
        ...
        cart.forEach((item, index) => {
          ...
          cartItem.innerHTML = `
            ...
            <img src="${adjustedImagePath}" alt="${
            item.name
          }" class="cart__item-img" />
            <div class="cart__item-details">
                <h3 class="cart__item-name">${item.name}</h3>
                <p class="cart__item-price">₹ ${parseFloat(item.price).toFixed(
                  2
                )}</p>
            </div>
            <div class="quantity-controls">
                <button class="decrease" data-index="${index}">-</button>
                <span class="quantity quantity-${index}">${
            item.quantity || 1
          }</span>
    ...
    function updateOrderSummary() {
      ...
      cart.forEach((item) => {
        ...
        subtotal += itemTotal;
        console.log(
          `Item: ${item.name}, Price: ${itemPrice}, Quantity: ${quantity}, Item Total: ${itemTotal}`
        );
      });
      ...
      console.log(
        `Subtotal: ${subtotal}, Tax: ${tax}, Delivery: ${deliveryCharge}, Discount: ${discount}, Total: ${total}`
      );
    ...
    function displayCurrentOrder() {
      ...
      orderRow.innerHTML = `
        ...
        <td><img src="${adjustedImagePath}" alt="${
      ...
      cancelOrderButton.style.display =
        currentOrder.status === "delivered" ? "none" : "block";
