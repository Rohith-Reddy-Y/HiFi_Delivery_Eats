...
def generate_line_chart():
    ...
    for agent in agents:
        ...
        for order in orders:
            if order.delivered_at and order.created_at:
                diff_minutes = (order.delivered_at - order.created_at).total_seconds() / 60  # minutes difference
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=agent_names, x=early_counts, name="Early", marker=dict(color='#1E3A8A'), orientation='h'))
    fig.add_trace(go.Bar(y=agent_names, x=on_time_counts, name="On Time", marker=dict(color='#6366F1'), orientation='h'))
    fig.add_trace(go.Bar(y=agent_names, x=late_counts, name="Late", marker=dict(color='#FF8C00'), orientation='h'))
...
def generate_agent_rating_chart():
    ...
    for agent in agents:
        for rating_value in range(1, 6):
            count = DeliveryFeedback.query.filter_by(delivery_agent_id=agent.delivery_agent_id, rating=rating_value).count()
...
def generate_customer_feedback_chart():
    ...
    avg_rating = (sum(r * c for r, c in feedback_counts.items()) / sum(counts)) if sum(counts) > 0 else 0
...
def generate_monthly_retention_chart():
    ...
    for month_num in range(1, 13):
        ...
        total_customers = db.session.query(Order.customer_id).filter(
            extract('month', Order.created_at) == month_num
        ).distinct().count()

        repeat_customers = 0
        unique_users = db.session.query(Order.customer_id).filter(
            extract('month', Order.created_at) == month_num
        ).distinct().all()

        for (customer_id,) in unique_users:
            order_count = db.session.query(Order.order_id).filter(
